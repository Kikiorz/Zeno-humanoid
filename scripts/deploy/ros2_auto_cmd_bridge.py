...
try:
    ...
    raise SystemExit(
        "Run this bridge with ROS2 Humble system Python, not the conda model Python. "
        "Example: source /opt/ros/humble/setup.bash && /usr/bin/python3 "
        "scripts/deploy/ros2_auto_cmd_bridge.py"
    ) from exc
...
class Ros2AutoCmdBridge(Node):
    def __init__(self) -> None:
        super().__init__("ros2_auto_cmd_bridge")
