#!/usr/bin/env bash
# Consumer-only raw-label training. The shared cache supervisor owns the
# memfd; this script must not invoke run_*_cached.sh or allocate another one.
set -euo pipefail
...
RUN_ID="${RUN_ID:-robot8_20260726_act_dinov3_4cam_640x480_headstereo_rectified_crop_lr_all23_decoder7_b32_100k}"
...
if [[ ! -d "${DATASET_ROOT}" || ! -x "${VENV_DIR}/bin/python" ]]; then
  printf 'Missing corrected dataset or environment. dataset=%s venv=%s\n' "${DATASET_ROOT}" "${VENV_DIR}" >&2
  exit 1
fi
if [[ -e "${OUTPUT_DIR}" ]]; then
  printf 'Refusing to overwrite raw training output: %s\n' "${OUTPUT_DIR}" >&2
  exit 1
fi
