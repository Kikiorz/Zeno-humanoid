#!/usr/bin/env bash
# Read-only acceptance audit for the V3 translation/rotation-separated labels.
#
# This intentionally does not create the dataset.  It must pass before the
# long-running cache/training supervisor job is allowed to allocate RAM/GPU.
set -euo pipefail
...
DATASET_REPO_ID="${DATASET_REPO_ID:-${SOURCE_REPO_ID}_base_mode_separated_v3}"
DATASET_ROOT="${DATASET_ROOT:-${LEROBOT_ROOT}/${DATASET_REPO_ID}}"
ANALYSIS_DIR="${ANALYSIS_DIR:-${REPO_ROOT}/outputs/analysis/robot8_20260726_base_mode_separated_v3_dryrun}"
...
if [[ -e "${DATASET_ROOT}" ]]; then
  printf 'V3 output already exists; dry-run will still audit it without touching it: %s\n' \
    "${DATASET_ROOT}"
fi
...
exec "${VENV_DIR}/bin/python" \
  ...
  --preview-episodes 3 \
  --dry-run \
  --dataset-version base_mode_separated_v3 \
  --metadata-dir-name base_mode_separated_v3 \
