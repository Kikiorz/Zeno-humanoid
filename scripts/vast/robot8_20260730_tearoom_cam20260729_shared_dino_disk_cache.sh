#!/usr/bin/env bash
# Build one durable fp16 DINOv3 cache for both 30 Hz label variants.
set -euo pipefail
...
if [[ ! -x "${VENV_DIR}/bin/python" || ! -d "${SOURCE_DATASET}" || ! -f "${DINO_PRETRAINED_WEIGHTS}" ]]; then
  printf 'Missing cache prerequisites. venv=%s source=%s weights=%s\n' \
    "${VENV_DIR}" "${SOURCE_DATASET}" "${DINO_PRETRAINED_WEIGHTS}" >&2
  exit 1
fi
if [[ -e "${CACHE_MANIFEST}" || -e "${CACHE_FILE}" ]]; then
  printf 'Refusing to overwrite DINO cache artifacts. manifest=%s cache=%s\n' \
...
printf '[%s] precomputing frozen DINOv3 cache with torchcodec on both GPUs\n' "$(date '+%F %T')"
