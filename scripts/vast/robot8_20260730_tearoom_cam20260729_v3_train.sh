#!/usr/bin/env bash
# GPU 1 V3-label ACT training. It shares the normal run's visual DINO cache.
set -euo pipefail
...
RUN_ID="${RUN_ID:-robot8_20260730_tearoom_act_dinov3_3cam_640x480_topcam_left_cam20260729_all23_base_anchor_odom_v3_decoupled_smooth_ops3_to_base3_to_equal_decoder7_b32_100k_seqcache}"
...
if [[ ! -d "${DATASET_ROOT}" || ! -d "${SOURCE_DATASET}" || ! -x "${VENV_DIR}/bin/python" || ! -f "${DINO_PRETRAINED_WEIGHTS}" ]]; then
  printf 'Missing V3-training prerequisites. dataset=%s source=%s venv=%s weights=%s\n' \
    "${DATASET_ROOT}" "${SOURCE_DATASET}" "${VENV_DIR}" "${DINO_PRETRAINED_WEIGHTS}" >&2
  exit 1
fi
...
repo_id = sys.argv[3]
source_dataset = Path(sys.argv[4])
